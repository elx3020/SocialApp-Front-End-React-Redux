...
const Notifications = (props) => {
    ...
    const displayNotifications = (notification) => {
        ...
        let sentence = notification.type === 'like' ? " like your post." : " comment your post";
    ...
    const existANotification = notifications.some(notification => notification.read === false);
    ...
    const toogleNotificationPanel = (event) => 
    {
        ...
        if(!isPanelOpen) {
            ...
            if(notifications.filter(notification => notification.read === false).length > 0){
                props.markNotificationRead(notifications.filter(notification => notification.read === false).map(notification => notification.notificationId))
            }
    ...
    const alertIcon = existANotification ? (<div className='alert-active'></div>) : (<div></div>);
    ...
    return(
    <div>
        <div style={notificationBase} onClick={toogleNotificationPanel}>Notifications</div>
        {alertIcon}
